...
// $path = '/var/www/linux.org/site/htroot/doc';
    ...
    if ($operation == 'list') {
        ...
        $cmd = 'java -jar pdfformfiller.jar ' . $file . ' -font "fonts/arial.ttf"' .
                $flatten . ' 2>&1';
...
function fill_pdf_file_1($pipe) {
    ...
    $error |= false === fwrite($pipe, 'fill_7' . ' ' . pdfff_escape($_POST['ADDRESS']) . "\n");
    $error |= false === fwrite($pipe, 'fill_9' . ' ' . pdfff_escape($_POST['PHONE'] . ', ' . $_POST['EMAIL']) . "\n");

    $error |= false === fwrite($pipe, 'change_identification' . ' ' . pdfff_checkbox($_POST['IDENTIFICATION']) . "\n");
    $error |= false === fwrite($pipe, 'change_adress' . ' ' . pdfff_checkbox($_POST['CHANGE_ADRESS']) . "\n");
